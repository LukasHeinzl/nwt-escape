<script lang="ts">
  ...
  let nextRoomBtnText: string;

  $: doorLocksInInventoryCount = $currentGameData.inventory.filter((obj: EscapeObject) => obj.type === "DoorLock").length;
  $: nextRoomBtnText = $currentRoom === $currentGameData.rooms.length - 1 ? "Finish game" : "Enter next room";
</script>
...
<ObjectOverlay title={objectData.name} bind:isOverlayVisible>
  {#if objectData.needsLock}
    ...
    {:else}
      <button on:click={() => enterNextRoom()}>{nextRoomBtnText}</button>
    {/if}
